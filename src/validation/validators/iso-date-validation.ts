...
export class ISODateValidation implements IValidation {
  ...
  private checkDate(input: any): boolean {
    ...
    const parsedDate = new Date(Date.parse(input[this.fieldName] as string))
    return isNaN(Date.parse(input[this.fieldName] as string)) || parsedDate.toISOString().substring(0, 10) !== input[this.fieldName]
  }
}
