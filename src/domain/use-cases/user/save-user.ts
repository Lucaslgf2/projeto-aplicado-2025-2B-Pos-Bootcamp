...
export class SaveUser implements ISaveUser {
  ...
  async save(params: NsSaveUser.Input): Promise<NsSaveUser.Output> {
    ...
    const passwordHash = params.password ? await this.hashComparer.hash(params.password) : undefined
    const newUser = await this.userRepo.insert({ ...params, password: passwordHash })

    const confirmToken = await this.authToken.signToken({ userId: newUser.userId, email: newUser.email }, 1 * 60 * 60)
    ...
    return { userId: newUser.userId, confirmToken }
  }
}
